...
export function Sidebar({ className }) {
  ...
  const handleVoiceInterviewClick = (e, href) => {
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
          <nav className="space-y-1">
            {navigationItems.map((item) => {
              ...
              const isVoiceInterviewItem = item.href === "/voice-mock-interview";

              const LinkWrapper = ({ children }) => {
                if (isVoiceInterviewItem) {
                  return (
                    <button
                      onClick={(e) => handleVoiceInterviewClick(e, item.href)}
                      aria-label={item.name}
                      className={cn(
                        "relative flex items-center gap-3.5 px-4 py-3 text-sm font-medium rounded-xl transition-colors duration-200 group w-full text-left",
                        active
      ...
      <PremiumRequiredModal
        ...
        featureName="AI Voice & Video Mock Interview"
      />
    </>
  );
}
